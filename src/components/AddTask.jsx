    ...
    <div className="w-full bg-white p-6 rounded-lg shadow-md mt-10">
      ...
      <form name="addform" className="flex items-center gap-4" onSubmit={handleSubmit}>
        <label htmlFor="reminder" className="flex items-center space-x-2">
            ...
            <input type="text" autoFocus ref={inputRef} id="addtask" name="addtask" placeholder="Add Task" className="flex-1 border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" value={newTask} onChange={(e)=>setNewTask(e.target.value)}/>            
            <button type="submit" onClick={(e) => inputRef.current.focus()} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-500 rounded hover:bg-blue-600 transition">
                <FaPlus/> Add Task
            </button>
      </form>
    </div>
